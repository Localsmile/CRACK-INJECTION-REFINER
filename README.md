...
- IndexedDB(Dexie) 기반 로어 팩, 백업 화면의 JSON 가져오기, 로어팩 관리의 이름 변경/활성화/내보내기
...
- OpenAI 호환은 커스텀 전체 URL(신규 기본값) / Chat Completions / Responses / Anthropic Messages 형식을 선택하고 기능별 모델명을 직접 입력함. 커스텀은 입력 URL을 그대로 호출하며, 호환 파라미터 탐색은 제한된 형태만 시도하고 성공 형태를 캐시
- 전체 대화 추출은 성공한 구간을 저장하고 실패한 구간 번호만 남겨 이후에 해당 구간만 다시 시도
- 자동/수동 추출은 읽기 범위와 추출 항목을 각각 지정하며, 선택적으로 나중에 회상할 핵심 대사를 저장
...
- 최상위 메뉴: 홈 · 로어 관리 · 로어 추출/변환 · 백업 · 응답 교정 · API 설정 · 활동 · 도움말
- 홈의 자동 로어 추출 상태에 실행 주기와 다음 추출까지 남은 턴을 함께 표시
- 로어팩 관리에서 팩과 개별 로어를 펼쳐 전체 JSON을 확인·수정할 수 있음
- 중복 정리는 합칠 로어를 직접 선택해 AI로 병합하며, 유사도 임계값은 선택적인 후보 필터로만 사용함. 병합/되돌리기 후 검색 준비는 자동 갱신
- OOC 출력 형식은 참고 맥락 / 간단 참고 / 커스텀으로 단순화하고, 로어가 캐릭터 행동이나 대사를 강제하지 않도록 기본 문구를 완화
- 팩 관리, 이름 변경/채팅별 활성화, 항목별 ON/OFF, 인라인 JSON 편집, 스냅샷 내용 확인
- 주입/추출 실행 로그 뷰어
- 쿨다운 상태 모니터 + 수동 해제
