  ...
  UI.registerPanel({
    ...
    render: function(panel) {
      ...
      UI.addBox(panel, function(nd) {
        nd.appendChild(UI.createSection('수동 검사', '이미 처리된 응답도 최신 AI 응답을 다시 검사'));
        ...
        btn.onclick = async function() {
          if (!R.manualRefine) { alert('수동 교정 검사 사용 불가'); return; }
          if (!C || !C.getCurrentChatId) { alert('채팅 감지 도구 사용 불가'); return; }
      ...
      UI.addBox(panel, function(nd) {
        nd.appendChild(UI.createSection('프롬프트 템플릿', '기본 템플릿, 주제 기반 동적 프롬프트, 직접 입력 프롬프트 중 선택'));
      ...
      UI.addBox(panel, function(nd) {
        ...
        btn.onclick = function(){ if (R.clearProcessed) R.clearProcessed(); alert('처리 기록을 비움'); };
        nd.appendChild(btn);
      });
    }
  });
