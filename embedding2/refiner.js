(function () {
  ...
  const TEMPLATES = {
    full: {
      ...
      desc: "로어 모순 파악과 코드블록/문장 끊김 복구를 동시에 수행",
      turnHint: 1,
      prompt: `You are a Chat Continuity Checker.
RATE: PG-13 ~ NC-21
...
- [New Speech]: the latest AI-generated chat response
...
7. CONTEXTUAL HONORIFIC MISMATCH:
   ...
   Do NOT flag when recent context plausibly justifies the switch: anger, emotional distance, sarcasm, teasing, public/formal setting, disguise, identity reveal, memory reset, possession, chat-within-chat, deliberate relationship shift, or quoted old dialogue.
...
B) STATUS BLOCK TRUNCATION (triple-backtick fenced code blocks ```...``` at the END of an assistant message, containing chat stats/HP/variables/scene info):
...
{message}`
    ...
    repairOnly: {
      ...
      desc: "모순 체크 없이 텍스트나 상태창의 끊김만 빠르게 복구",
      turnHint: 0,
      prompt: `You are a Chat Text Repair AI.
RATE: PG-13 ~ NC-21
...
{message}`
    ...
    logicOnly: {
      ...
      desc: "수집된 로어를 바탕으로 논리적 모순만 엄격히 체크",
      turnHint: 1,
      prompt: `You are a Chat Logic Checker.
RATE: PG-13 ~ NC-21
...
{message}`
  ...
  const _REPAIR_BLOCKS = {
    ...
    truncStatusBlock: 'B) STATUS BLOCK TRUNCATION (triple-backtick fenced code blocks ```...``` at the END of an assistant message, containing chat stats/HP/variables/scene info):\n   Restore truncated/absent status blocks using [Recent Context]. Copy the most recent assistant-side code block verbatim (including the triple backticks). Only update numeric values if [New Speech] explicitly states them.',
  };
  ...
  function buildDynamicPrompt(topics) {
    ...
    if (!hasLogic && !hasRepair) {
      return 'You are a Chat Checker. All topics disabled — always output {passWord}.\n\n[New Speech]:\n{message}';
    }
    let p = 'You are a Chat Continuity Checker.\nRATE: PG-13 ~ NC-21\n\n## Scope\nYou receive:\n- [Lore]: partial character/world data\n- [Summaries]: memory snapshots\n- [Recent Context]: last N turns of dialogue\n- [New Speech]: the latest AI-generated chat response\n\n## Rule\nFlag an error ONLY when [New Speech] DIRECTLY CONTRADICTS a fact explicitly stated in [Lore] or [Summaries].\n';
...
`You are a Chat Continuity Checker.
RATE: PG-13 ~ NC-21
