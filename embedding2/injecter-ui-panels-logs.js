  ...
  function addLogSection(panel, title, color, items, clearFn, renderItem) {
    UI.addBox(panel, function(nd) {
      ...
      const detail = UI.createDetailsSection
        ? UI.createDetailsSection(title + ' (' + sourceItems.length + ')', '최근 기록을 필요할 때만 펼쳐서 확인', false)
        : null;
      if (detail) {
  ...
  UI.registerPanel({
    ...
    render: function(panel) {
      ...
      addLogSection(panel, '주입 기록', '#4a9', inj, function(){ if (R.clearInjLog) R.clearInjLog(chatKey); }, function(item, row) {
        row.textContent = (item.time || '') + ' 턴 ' + (item.turn || '?') + ' / ' + (item.count || 0) + ' 항목 / ' + ((item.matched || []).join(', ') || item.note || '일치 없음');
      });

      addLogSection(panel, '검색 진단', '#7fd7bd', inj.filter(item => item && item.diagnostics), function(){ if (R.clearInjLog) R.clearInjLog(chatKey); }, function(item, row) {
        const d = item.diagnostics || {};
        const st = d.searchStats || {};
        const top = (d.candidates || []).slice(0, 5).map(c => {
          const bits = [c.name, '점수 ' + c.score];
          if (c.trigger) bits.push('트리거 ' + c.trigger + (c.triggerType ? '(' + c.triggerType + ')' : ''));
          if (c.semantic) bits.push('의미 ' + c.semantic);
          if (c.llmScore) bits.push('재정렬 ' + c.llmScore);
          return bits.join(' / ');
        }).join('\n');
        const finalNames = (d.final || []).map(x => x.name).join(', ') || '없음';
        row.textContent =
          '[' + (item.time || '') + '] 턴 ' + (item.turn || '?') +
          '\n키워드만 ' + (st.keywordOnly || st.trigOnly || 0) +
          ' / 의미만 ' + (st.semanticOnly || st.embOnly || 0) +
          ' / 혼합 ' + (st.mixed || st.both || 0) +
          ' / 누락벡터 ' + (st.missingVector || 0) +
          ' / stale ' + (st.semanticSkippedStale || 0) +
          ' / 모델불일치 ' + (st.semanticSkippedModel || 0) +
          '\n최종 주입: ' + finalNames +
          (top ? '\n후보:\n' + top : '');
      });

      addLogSection(panel, '추출 기록', '#da8', ext, function(){ if (R.clearExtLog) R.clearExtLog(chatKey); }, function(item, row) {
        row.textContent = '[' + (item.time || '') + '] ' + (item.isManual ? '수동' : '자동') + ' / ' + (item.status || '?') + ' / ' + (item.count || 0) + ' 항목' + (item.error ? ' / ' + item.error : '');
      });
      ...
      addLogSection(panel, '응답 교정 기록', '#ea5', ref, function(){
        ...
        row.textContent = '[' + (item.time || '') + '] ' + (item.isPass ? '통과' : item.isError ? '오류' : '교정') + (item.reason ? ' / ' + item.reason : '');
      });
  ...
  UI.registerPanel({
    ...
    render: async function(panel) {
      ...
      if (!settings || !settings.config || !db) {
        UI.addBox(panel, function(nd){ nd.appendChild(UI.createNotice('설정이나 데이터베이스를 아직 불러오지 못했음', 'error')); });
        return;
      }
      ...
      UI.addBox(panel, function(nd) {
        nd.appendChild(UI.createSection('세션 상태', '현재 채팅 턴, 활성 팩, 쿨다운 상태 확인'));
        ...
        reset.onclick = async function() {
          const ok = await UI.createConfirmDialog({
            title: '세션 상태 초기화',
            message: '이 채팅의 쿨다운, 마지막 언급, 턴 카운터를 초기화할까요?',
            danger: true,
            confirmLabel: '초기화',
            cancelLabel: '취소'
          });
          ...
          save(settings);
          await UI.createAlertDialog({ title: '초기화 완료', message: '세션 상태를 초기화했어요.', tone: 'success' });
          location.reload();
        };
        const row = document.createElement('div');
      ...
      UI.addBox(panel, function(nd) {
        ...
        if (UI.createDetailsSection) {
          const detail = UI.createDetailsSection('쿨다운 항목', '현재 채팅에서 잠시 제외된 로어 항목', false);
        ...
        ids.forEach(function(id) {
          ...
          text.textContent = (entry ? '[' + (entry.type || '항목') + '] ' + entry.name : id) + ' / 마지막 턴 ' + cooldown[id];
