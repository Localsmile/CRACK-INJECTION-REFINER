...
(async function(){
  ...
  function setStatus(owner, msg, opts) {
    try {
      if (C.status && C.status.set) C.status.set(owner, msg, opts || { busy: true });
      else C.showStatusBadge(msg);
    } catch(e) {}
  }
  function clearStatus(owner) {
    try {
      if (C.status && C.status.clear) C.status.clear(owner);
  ...
  async function inject(userInput) {
    ...
    let scored = [], activeNames = [], searchStats = null;
    try {
      const r = await C.hybridSearch(userInput, recentMsgs, enabled, searchConfig, apiOpts);
      scored = r.scored || [];
      activeNames = r.activeNames || [];
      searchStats = r.searchStats || null;
    ...
    if (config.pendingPromiseBoost !== false) {
      for (const s of scored) {
        const isPromise = C.isPromiseEntry ? C.isPromiseEntry(s.entry) : (s.entry.type === 'promise' || s.entry.type === 'prom');
        const status = C.getPromiseStatus ? C.getPromiseStatus(s.entry) : (s.entry.state || s.entry.detail?.status || '');
        if (isPromise && /pending|대기|진행|미완|약속|유지/i.test(String(status || ''))) {
          s.score = Math.max(s.score, 0.3);
          s.scoreParts = Object.assign({}, s.scoreParts || {}, { pendingPromiseBoost: true, final: s.score });
        }
      }
    ...
    const candidatesBeforeRerank = scored.slice(0, 12).map(s => ({
      id: s.entry.id,
      name: s.entry.name,
      type: s.entry.type,
      score: Number((s.score || 0).toFixed(4)),
      keyword: Number((s.tScore || 0).toFixed(4)),
      semantic: Number((s.eSim || 0).toFixed(4)),
      trigger: s.matchedTrigger || '',
      triggerType: s.matchedTriggerType || '',
      parts: s.scoreParts || null
    }));

    if (config.rerankEnabled) {
      try {
        setStatus('rerank', '에리가 로어 재정렬 중', { busy: true });
      ...
      finally { clearStatus('rerank'); }
      // 리랭크 직후 hide 대신 "응답 기다리는 중"으로 전환 — Refiner가 실제 응답 감지 시 다음 상태로 교체/hide 담당
      if (config.refinerEnabled) setStatus('refiner', '에리가 응답 기다리는 중', { busy: true });
    }
    ...
    const candidateDiagnostics = _filteredScored.slice(0, 12).map(s => ({
      id: s.entry.id,
      name: s.entry.name,
      type: s.entry.type,
      score: Number((s.score || 0).toFixed(4)),
      origScore: s.origScore !== undefined ? Number((s.origScore || 0).toFixed(4)) : undefined,
      llmScore: s.llmScore,
      keyword: Number((s.tScore || 0).toFixed(4)),
      semantic: Number((s.eSim || 0).toFixed(4)),
      trigger: s.matchedTrigger || '',
    ...
    if (!topEntries.length) {
      addInjLog(chatKey, {
        ...
        matched: [], count: 0, note: '검색 후보 없음',
        diagnostics: { searchStats, activePacks: activePacksArr, activeNames, candidatesBeforeRerank, candidates: candidateDiagnostics }
      });
      return userInput;
    }
    ...
    addInjLog(chatKey, {
      ...
      diagnostics: {
        searchStats,
        activePacks: activePacksArr,
        activeNames,
        candidatesBeforeRerank,
        candidates: candidateDiagnostics,
        final: fmtResult.included.map(e => ({ id: e.id, name: e.name, type: e.type })),
        budgetFit: {
          userInputChars: _userLen,
          injectedChars: _injectedLen,
          totalChars: _userLen + _injectedLen + 2,
