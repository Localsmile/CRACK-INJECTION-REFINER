...
(function(){
  ...
  function embeddingApiOpts(settings) {
  ...
  async function countMissingSearchRows(db, entries) {
    const ids = entries.map(e => e.id).filter(id => id !== undefined && id !== null);
    if (!ids.length || !db.embeddings) return entries.length;
    let rows = [];
    try { rows = await db.embeddings.where('entryId').anyOf(ids).toArray(); }
    catch(_) {
      for (const id of ids) {
        const part = await db.embeddings.where('entryId').equals(id).toArray();
        rows.push.apply(rows, part);
      }
    }
    const have = new Set(rows.filter(r => r && r.field === 'summary' && r.vector).map(r => r.entryId));
    return entries.filter(e => !have.has(e.id)).length;
  }

  function renderSearchDataTools(panel, db, settings, C, activePacks, allEntries) {
    UI.addBox(panel, async function(nd) {
      nd.appendChild(UI.createSection(
        '검색 데이터',
        '의미 기반 검색용 데이터를 확인하고 활성 팩 기준으로 재생성',
        HELP.searchStale
      ));

      const activeEntries = allEntries.filter(e => activePacks.includes(e.packName));
      const stale = activeEntries.filter(e => e.searchStale);
      const missing = await countMissingSearchRows(db, activeEntries);
      nd.appendChild(UI.createSummaryGrid([
        { label: '활성 항목', value: String(activeEntries.length) },
        { label: '갱신 필요', value: String(stale.length) },
        { label: '검색 데이터 없음', value: String(missing) }
      ]));

      if (!activePacks.length || !activeEntries.length) {
        nd.appendChild(UI.createNotice('활성 팩과 항목이 있어야 검색 데이터를 만들 수 있어요.', 'warn'));
        return;
      }

      const status = UI.createInlineStatus('', 'info');
      const rebuild = UI.createButton('활성 팩 검색 데이터 재생성', { kind: 'primary', bold: true });
      const clearStale = UI.createButton('갱신 표시만 정리', { compact: false });

      rebuild.onclick = async function() {
        if (!C || !C.embedPack) {
          await UI.createAlertDialog({ title: '재생성 불가', message: '검색 데이터 생성 도구를 불러오지 못했어요.', tone: 'error' });
          return;
        }
        if (!hasEmbeddingApi(settings)) {
          await UI.createAlertDialog({ title: '임베딩 API 필요', message: 'API 설정에서 임베딩용 연결값을 먼저 입력하세요.', tone: 'warn' });
          return;
        }
        const ok = await UI.createConfirmDialog({
          title: '검색 데이터 재생성',
          message: activePacks.length + '개 활성 팩의 검색 데이터를 다시 만들까요?\n항목 수가 많으면 시간이 걸릴 수 있어요.',
          confirmLabel: '재생성',
          cancelLabel: '취소'
        });
        if (!ok) return;
        rebuild.disabled = true;
        const opts = embeddingApiOpts(settings);
        let total = 0;
        try {
          for (const packName of activePacks) {
            status.setStatus(packName + ' 재생성 중...', 'info');
            const done = await C.embedPack(packName, opts, function(cur, max) {
              status.setStatus(packName + ' ' + cur + '/' + max + ' 생성 중...', 'info');
            });
            total += done || 0;
          }
          status.setStatus('완료: ' + total + '개 항목 검색 데이터 생성', 'success');
          await UI.createAlertDialog({ title: '재생성 완료', message: total + '개 항목의 검색 데이터를 만들었어요.', tone: 'success' });
          location.reload();
        } catch(e) {
          status.setStatus('실패: ' + String(e.message || e), 'error');
        } finally {
          rebuild.disabled = false;
        }
      };

      clearStale.onclick = async function() {
        const ok = await UI.createConfirmDialog({
          title: '갱신 표시 정리',
          message: '검색 데이터가 이미 있다고 보고 stale 표시만 끌까요?\n실제 데이터가 없으면 의미 검색 품질은 좋아지지 않아요.',
          confirmLabel: '정리',
          cancelLabel: '취소'
        });
        if (!ok) return;
        for (const e of stale) {
          try { await db.entries.update(e.id, { searchStale: false, searchUpdatedAt: Date.now() }); } catch(_){}
        }
        await UI.createAlertDialog({ title: '정리 완료', message: '갱신 필요 표시를 정리했어요.', tone: 'success' });
        location.reload();
      };

      nd.appendChild(UI.createActionGroup([rebuild], [clearStale], []));
  ...
  UI.registerPanel({
    ...
    render: async function(panel) {
      ...
      renderSearchDataTools(panel, db, settings, C, enabled, allEntries);
