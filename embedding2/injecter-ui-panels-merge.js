...
(function(){
  ...
  function undoStorageKey() {
    try {
      ...
      const url = C && C.getCurUrl ? C.getCurUrl() : location.href;
      return 'lore-last-merge-undo:' + url;
    } catch(_) {
      return 'lore-last-merge-undo:' + location.href;
    }
  }
  function savePersistentMergeUndo(C, undo) {
    if (C) C.__lastMergeUndo = undo;
    try { localStorage.setItem(undoStorageKey(), JSON.stringify(undo)); } catch(_) {}
  }
  function loadPersistentMergeUndo(C) {
    if (C && C.__lastMergeUndo) return C.__lastMergeUndo;
    try {
      const raw = localStorage.getItem(undoStorageKey());
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      if (C) C.__lastMergeUndo = parsed;
      return parsed;
    } catch(_) {
      return null;
    }
  }
  function clearPersistentMergeUndo(C) {
    if (C) C.__lastMergeUndo = null;
    try { localStorage.removeItem(undoStorageKey()); } catch(_) {}
  }
  ...
  async function executeMerge(group, draft, db, settings, C) {
    ...
    savePersistentMergeUndo(C, { mergedId: target.id, originals, ts: Date.now() });
  ...
  async function undoLastMerge(db, C) {
    const undo = loadPersistentMergeUndo(C);
  ...
  async function renderDuplicateMerge(panel) {
      ...
      UI.addBox(panel, function(nd) {
        ...
        undo.onclick = async function() {
          if (!loadPersistentMergeUndo(C)) {
            await UI.createAlertDialog({ title: '되돌릴 기록 없음', message: '저장된 마지막 병합 되돌리기 기록이 없어요.', tone: 'warn' });
            return;
          }
          const ok = await UI.createConfirmDialog({
            title: '마지막 병합 되돌리기',
            message: '마지막 병합을 되돌릴까요?\n새로고침 뒤에도 남아 있는 복구 기록을 사용해요.',
            danger: true,
            confirmLabel: '되돌리기',
            cancelLabel: '취소'
          });
