...
(function(){
  ...
  async function scanGroups(db, settings, C) {
    ...
    if (entries.length < 2) return { entries, groups: [], reason: '활성 항목이 2개 이상 필요합니다.' };
    ...
    if (withEmb.length < 2) return { entries, groups: [], reason: '검색 데이터가 있는 항목이 2개 이상 필요합니다.' };
  ...
  async function undoLastMerge(db, C) {
    ...
    if (!undo || !undo.originals || !undo.originals.length) throw new Error('현재 실행 세션에 되돌릴 병합 기록이 없습니다.');
  ...
  function renderGroup(panel, group, index, db, settings, C, rerender) {
    UI.addBox(panel, function(nd) {
      ...
      if (types.length > 1) {
        nd.appendChild(UI.createNotice('유형이 서로 다릅니다: ' + types.join(', ') + '. 병합 전에 꼭 확인하세요.', 'warn'));
      }
      ...
      async function buildPreview() {
        ...
        if (mode.value === 'llm-summarize') {
          if (!C || !C.callGeminiApi) throw new Error('LLM API 도구를 불러오지 못했습니다.');
          if (!hasApi(settings, false)) throw new Error('LLM API 설정이 없습니다.');
      ...
      mergeBtn.onclick = async function() {
        try {
          ...
          const summaryLen = String(group.draft.summary || '').length;
          if (summaryLen > state.maxChars && !confirm('요약이 최대 길이를 넘었습니다. 계속할까요?')) return;
          if (types.length > 1 && !confirm('유형이 서로 다릅니다. 그래도 병합할까요?')) return;
          if (!confirm(group.entries.length + '개 항목을 하나로 병합할까요?')) return;

          mergeBtn.disabled = true;
          const result = await executeMerge(group, group.draft, db, settings, C);
          alert('병합 완료. ' + result.embedMsg);
  ...
  UI.registerPanel({
    ...
    render: async function(panel) {
      ...
      const rerender = function() {
        state.message = state.message || '패널 상태가 바뀌었습니다. 결과를 새로 보려면 이 패널을 다시 여세요.';
      };

      UI.addBox(panel, function(nd) {
        nd.appendChild(UI.createSection('중복 병합', '기존 검색 데이터를 사용해 활성 로어의 중복 후보를 찾습니다.'));
        nd.appendChild(UI.createNotice('이 도구는 저장된 로어 데이터를 수정합니다. 병합 전 스냅샷을 만드세요.', 'warn'));
        ...
        scan.onclick = async function() {
          ...
          try {
            ...
            state.message = result.groups.length ? '후보 그룹 ' + result.groups.length + '개를 찾았습니다.' : (result.reason || '후보 없음');
        ...
        undo.onclick = async function() {
          if (!C || !C.__lastMergeUndo) { alert('현재 실행 세션에 되돌릴 병합 기록이 없습니다.'); return; }
          if (!confirm('현재 실행 세션의 마지막 병합을 되돌릴까요?')) return;
          undo.disabled = true;
          try {
      ...
      if (!state.groups.length) {
        UI.addBox(panel, function(nd) {
          nd.appendChild(UI.createNotice('현재 기준에서 병합 후보가 없습니다.', 'info'));
        });
        return;
      }
