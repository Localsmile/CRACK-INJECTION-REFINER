...
(function(){
  ...
  function createButton(label, opts) {
    ...
    btn.style.cssText = 'padding:' + (opts.compact ? '4px 10px' : '8px 14px') + ';font-size:12px;border-radius:6px;cursor:pointer;background:' + bg + ';color:' + color + ';border:1px solid ' + border + ';font-weight:' + (opts.bold ? 'bold' : 'normal') + ';transition:background-color .12s ease,border-color .12s ease,transform .08s ease,opacity .12s ease;box-shadow:none;';
    btn.addEventListener('mouseenter', function() {
      if (btn.disabled) return;
      btn.style.opacity = '0.92';
      if (!opts.kind) btn.style.background = '#181818';
    });
    btn.addEventListener('mouseleave', function() {
      btn.style.opacity = '';
      btn.style.transform = '';
      if (!opts.kind) btn.style.background = 'transparent';
    });
    ...
    btn.addEventListener('focus', function() {
      btn.style.outline = '2px solid rgba(68,170,153,.45)';
