...
(async function(){
  ...
  function safeSnapshotLabel(label) {
    return String(label || '자동 저장').slice(0, 120);
  }

  function cleanSnapshotEntry(entry) {
    const c = Object.assign({}, entry || {});
    delete c.id;
    return c;
  }

  function validateSnapshotData(snap) {
    if (!snap || !snap.packName || !Array.isArray(snap.data)) return '스냅샷 구조가 올바르지 않음';
    if (snap.data.some(e => !e || typeof e !== 'object' || !e.name)) return '스냅샷에 이름 없는 항목이 포함됨';
    return '';
  }

  async function createSnapshot(packName, label, type = 'auto') {
    if (!packName) throw new Error('스냅샷 대상 팩이 없음');
    const entries = await db.entries.where('packName').equals(packName).toArray();
    const clean = entries.map(cleanSnapshotEntry);
    const id = await db.snapshots.add({
      packName,
      timestamp: Date.now(),
      label: safeSnapshotLabel(label),
      type,
  ...
  function markPackEmbeddingsStale(packName) {
    if (!settings.config.packSearchStale) settings.config.packSearchStale = {};
    settings.config.packSearchStale[packName] = { stale: true, updatedAt: Date.now() };
    settings.save();
  }

  function clearPackEmbeddingStale(packName) {
    if (!settings.config.packSearchStale) settings.config.packSearchStale = {};
    delete settings.config.packSearchStale[packName];
    settings.save();
  }

  function cleanupDeletedPackReferences(packName, deletedEntryIds) {
    ...
    Object.keys(up).forEach(url => { up[url] = (up[url] || []).filter(name => name !== packName); });
    Object.keys(ud).forEach(url => { ud[url] = (ud[url] || []).filter(id => !deleted.has(id)); });
    settings.config.urlPacks = up;
    settings.config.urlDisabledEntries = ud;
    if (settings.config.packSearchStale) delete settings.config.packSearchStale[packName];
    settings.save();
  }
  ...
  async function restoreSnapshot(snapshotId) {
    ...
    const invalid = validateSnapshotData(snap);
    if (invalid) throw new Error(invalid);
    await createSnapshot(snap.packName, '복원 전 자동 백업 ' + new Date().toLocaleString(), 'pre_restore');
    await db.transaction('rw', db.packs, db.entries, db.embeddings, async () => {
      const oldEntries = await db.entries.where('packName').equals(snap.packName).toArray();
      const oldIds = oldEntries.map(e => e.id).filter(id => id !== undefined && id !== null);
      for (const id of oldIds) {
        try { await db.embeddings.where('entryId').equals(id).delete(); } catch(_) {}
      }
      await db.entries.where('packName').equals(snap.packName).delete();
      for (const raw of snap.data) {
        const clean = cleanSnapshotEntry(raw);
        const e = Object.assign({}, clean, { packName: snap.packName, searchStale: true, searchStaleAt: Date.now() });
        await db.entries.add(e);
      }
      const pack = await db.packs.get(snap.packName);
      if (pack) await db.packs.update(snap.packName, { entryCount: snap.data.length });
      else await db.packs.put({ name: snap.packName, entryCount: snap.data.length, project: '' });
    });
    markPackEmbeddingsStale(snap.packName);
    return true;
  }
  ...
  async function getAutoExtPackForUrl(url) {
    ...
    let baseName = settings.config.autoExtPack || '자동추출';
    try {
      let chatId = C.getCurrentChatId();
      if (!chatId) { const match = String(url || '').match(/\/episodes\/([a-f0-9]+)/); if (match) chatId = match[1]; }
      if (chatId) {
        const room = await CrackUtil.chatRoom().roomData(chatId);
        if (room && !(room instanceof Error)) baseName = room.story?.name || room.title || baseName || '자동추출';
      }
    } catch(e) {}

    const existing = Object.values(settings.config.urlAutoExtPacks || {});
    let finalName = baseName || '자동추출';

    if (existing.includes(finalName)) {
      let counter = 2;
      while (existing.includes(finalName + ' ' + counter)) counter++;
  ...
  Object.assign(_w.__LoreInj, {
    ...
    parseJsonLoose, createSnapshot, restoreSnapshot, deleteSnapshot,
    cleanupDeletedPackReferences, markPackEmbeddingsStale, clearPackEmbeddingStale,
