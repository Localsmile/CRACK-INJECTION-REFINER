...
(async function(){
  ...
  const defaultSettings = {
    ...
    refinerHonorificSensitivity: 'low', refinerPromptVersion: R ? (R.PROMPT_VERSION || '') : '',
    statusMotion: 'text', refinerModalSize: 'large', refinerRenderMode: 'safe', uiDensity: 'comfortable',
    urlRefinerLogs: {}
  };
        ...
        try {
          ...
          if (R2 && R2.PROMPT_VERSION && R2.LEGACY_PROMPTS) {
            ...
            if (savedVer !== R2.PROMPT_VERSION) {
              const isLegacy = !saved || (R2.isLegacyPrompt ? R2.isLegacyPrompt(saved) : R2.LEGACY_PROMPTS.some(p => norm(p) === norm(saved) || norm(saved).includes(norm(p))));
