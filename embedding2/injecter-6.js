  ...
  function setupUI() {
    ...
    modal.createMenu('로어 설정', (m) => {
      m.replaceContentPanel(async (panel) => {
        ...
        if (C.createShortcutRow && C.createSectionAnchor) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            nd.appendChild(C.createShortcutRow([
              { label: '빠른 시작', target: 'settings-presets' },
              { label: '기본', target: 'settings-basic' },
              { label: '검색', target: 'settings-search' },
              { label: '출력', target: 'settings-output' },
              { label: '초기화', target: 'settings-reset' }
            ], { marginBottom: '0' }));
            if (C.createSmallDivider) nd.appendChild(C.createSmallDivider({ margin: '8px 0 0' }));
          }});
        }
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          if (C.createSectionAnchor) nd.appendChild(C.createSectionAnchor('settings-presets'));
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          if (C.createSectionAnchor) nd.appendChild(C.createSectionAnchor('settings-basic'));
          ...
          const cmpWrap = document.createElement('div'); cmpWrap.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;padding-left:10px;';
          const cmpLbl = document.createElement('div'); cmpLbl.textContent = '압축 모드'; cmpLbl.style.cssText = 'font-size:12px;color:#aaa;';
          const cmpSel = document.createElement('select'); cmpSel.style.cssText = 'width:min(120px,100%);max-width:100%;padding:4px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:11px;box-sizing:border-box;';
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          if (C.createSectionAnchor) nd.appendChild(C.createSectionAnchor('settings-search'));
          ...
          const emRow = document.createElement('div'); emRow.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;gap:10px;width:100%;margin-bottom:8px;';
          ...
          const emSel = document.createElement('select'); emSel.style.cssText = 'width:min(200px,100%);max-width:100%;padding:6px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;';
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          const wrap = document.createElement('div'); wrap.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;width:100%;';
          ...
          const right = document.createElement('div'); right.style.cssText = 'display:flex;gap:6px;flex-wrap:wrap;';
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          if (C.createSectionAnchor) nd.appendChild(C.createSectionAnchor('settings-output'));
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          if (C.createSectionAnchor) nd.appendChild(C.createSectionAnchor('settings-reset'));
    ...
    .createSubMenu('로어 관리 (목록)', (m) => {
      const renderPanel = async (panel) => {
        ...
        if (!activeEntries.length) { panel.addText('활성 항목 없음.'); return; }

        const listUiState = renderPanel._listUiState || (renderPanel._listUiState = { query: '', type: 'all', status: 'all', pack: 'all' });
        const rerenderList = (delay = 0) => {
          if (renderPanel._filterTimer) clearTimeout(renderPanel._filterTimer);
          renderPanel._filterTimer = setTimeout(() => m.replaceContentPanel(renderPanel, '로어 목록 관리'), delay);
        };
        const getEntryEmbeddingState = async (e) => {
          const emb = await db.embeddings.where({entryId: e.id, field: 'summary'}).first() || await db.embeddings.where('entryId').equals(e.id).first();
          if (!emb) return 'missing';
          let h = 0;
          const s = e.embed_text ? (e.name + ' ' + e.embed_text) : (e.name + ': ' + (e.summary || ''));
          for (let i = 0; i < s.length; i++) h = ((h << 5) - h + s.charCodeAt(i)) | 0;
          const ch = h.toString(36);
          const targetModel = settings.config.embeddingModel || 'gemini-embedding-001';
          if (emb.hash !== ch) return 'stale';
          const needsRegen = (emb.model && emb.model !== targetModel) || (emb.taskType !== 'RETRIEVAL_DOCUMENT' && targetModel.includes('embedding-001'));
          return needsRegen ? 'stale' : 'embedded';
        };

        const q = (listUiState.query || '').trim().toLowerCase();
        let filtered = activeEntries.filter(e => {
          if (q) {
            const haystack = [
              e.name || '',
              e.summary || '',
              Array.isArray(e.triggers) ? e.triggers.join(' ') : (e.triggers || '')
            ].join(' ').toLowerCase();
            if (!haystack.includes(q)) return false;
          }
          if (listUiState.type !== 'all' && e.type !== listUiState.type) return false;
          if (listUiState.pack !== 'all' && e.packName !== listUiState.pack) return false;
          if (listUiState.status === 'enabled' && !isEntryEnabledForUrl(e)) return false;
          if (listUiState.status === 'disabled' && isEntryEnabledForUrl(e)) return false;
          return true;
        });

        if (['embedded', 'missing', 'stale'].includes(listUiState.status)) {
          const statusFiltered = [];
          for (const e of filtered) {
            const embState = await getEntryEmbeddingState(e);
            if (embState === listUiState.status) statusFiltered.push(e);
          }
          filtered = statusFiltered;
        }

        if (C.createListToolbar && C.createSearchInput && C.createFilterRow && C.createFilterSelect && C.createCountLabel) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            const toolbar = C.createListToolbar({ marginBottom: '4px' });
            toolbar.appendChild(C.createSearchInput({
              placeholder: '검색어 입력',
              value: listUiState.query,
              onInput: (v) => { listUiState.query = v; rerenderList(180); }
            }));

            const filterRow = C.createFilterRow();
            const typeOptions = [{ value: 'all', label: '전체 타입' }].concat(
              Array.from(new Set(activeEntries.map(e => e.type).filter(Boolean))).sort().map(type => ({ value: type, label: type }))
            );
            const packOptions = [{ value: 'all', label: '전체 팩' }].concat(
              Array.from(new Set(activeEntries.map(e => e.packName).filter(Boolean))).sort().map(pack => ({ value: pack, label: pack }))
            );
            filterRow.appendChild(C.createFilterSelect({
              value: listUiState.type,
              options: typeOptions,
              onChange: (v) => { listUiState.type = v; rerenderList(); }
            }));
            filterRow.appendChild(C.createFilterSelect({
              value: listUiState.status,
              options: [
                { value: 'all', label: '전체 상태' },
                { value: 'enabled', label: '활성' },
                { value: 'disabled', label: '비활성' },
                { value: 'embedded', label: '임베딩 완료' },
                { value: 'missing', label: '미생성' },
                { value: 'stale', label: '재생성 필요' }
              ],
              onChange: (v) => { listUiState.status = v; rerenderList(); }
            }));
            filterRow.appendChild(C.createFilterSelect({
              value: listUiState.pack,
              options: packOptions,
              onChange: (v) => { listUiState.pack = v; rerenderList(); }
            }));
            if (C.createCompactActionButton) {
              filterRow.appendChild(C.createCompactActionButton('초기화', {
                tone: 'muted',
                onClick: () => {
                  listUiState.query = '';
                  listUiState.type = 'all';
                  listUiState.status = 'all';
                  listUiState.pack = 'all';
                  rerenderList();
                }
              }));
            }
            toolbar.appendChild(filterRow);
            toolbar.appendChild(C.createCountLabel('표시 ' + filtered.length + '개 / 전체 ' + activeEntries.length + '개'));
            nd.appendChild(toolbar);
          }});
        }
        ...
        if (!filtered.length) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            if (C.createEmptyState) nd.appendChild(C.createEmptyState('조건에 맞는 항목이 없습니다.', '검색어나 필터를 바꿔보세요.'));
            else nd.textContent = '조건에 맞는 항목이 없습니다.';
          }});
          return;
        }
        ...
        for (const [pk, items] of Object.entries(byPack)) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            ...
            const headerRow = document.createElement('div'); headerRow.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;border-bottom:1px solid #333;padding-bottom:6px;cursor:pointer;';
            ...
            for (const e of items) {
              ...
              const header = document.createElement('div'); header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;';
              ...
              const moreBtn = document.createElement('button'); moreBtn.textContent = '추가'; moreBtn.style.cssText = B;
              ...
              const _renderAnchor = () => { const on = !!e.anchor; anchorBtn.textContent = on ? '앵커 적용' : '앵커'; anchorBtn.title = on ? '앵커 해제 — 자동 추출 병합 시 보호 해제됨' : '앵커 지정 — summary/state/detail/call/inject 자동 덮어쓰기 차단, 재주입 우선도 최대'; anchorBtn.style.cssText = B + (on ? 'color:#fc4;border-color:#963;background:#2a1a00;' : 'color:#777;border-color:#444;'); };
              _renderAnchor();
              ...
              actionTray.style.cssText = 'display:none;gap:6px;flex-wrap:wrap;justify-content:flex-end;margin-top:8px;padding-top:8px;border-top:1px dashed #222;';
              moreBtn.onclick = (ev) => { ev.stopPropagation(); const open = actionTray.style.display !== 'none'; actionTray.style.display = open ? 'none' : 'flex'; moreBtn.textContent = open ? '추가' : '닫기'; };
              right.appendChild(editBtn); right.appendChild(moreBtn);
              actionTray.appendChild(embGenBtn); actionTray.appendChild(copyBtn); actionTray.appendChild(histBtn); actionTray.appendChild(anchorBtn); actionTray.appendChild(delBtn);
              header.appendChild(left); header.appendChild(right); row.appendChild(header); row.appendChild(actionTray);
              ...
              const btnRow = document.createElement('div'); btnRow.style.cssText = 'display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap;';
      ...
      const renderSnapshotUI = async (panel) => {
        panel.addBoxedField('', '', { onInit: async (nd) => {
          ...
          for (const s of snaps) {
            const row = document.createElement('div'); row.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;padding:6px;background:#1a1a1a;border:1px solid #333;border-radius:4px;';
      ...
      const renderPackUI = async (panel) => {
        ...
        panel.addBoxedField('', '', { onInit: async (nd) => {
          ...
          for (const pack of packs) {
            ...
            const header = document.createElement('div'); header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;padding:10px 12px;background:#111;';
    ...
    .createSubMenu('대화 추출 및 변환', (m) => {
      const renderAutoUI = (panel) => {
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          const row1 = document.createElement('div'); row1.style.cssText = 'display:flex;gap:12px;margin-bottom:8px;align-items:center;flex-wrap:wrap;';
          ...
          const row2 = document.createElement('div'); row2.style.cssText = 'display:flex;gap:12px;margin-bottom:12px;align-items:center;flex-wrap:wrap;';
          ...
          const inputWrap = document.createElement('div'); inputWrap.style.cssText = 'display:flex;gap:6px;flex-wrap:wrap;';
          ...
          const s3 = document.createElement('select'); s3.style.cssText = 'width:min(100px,100%);max-width:100%;padding:6px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;';
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          const tplHeader = document.createElement('div'); tplHeader.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;';
    ...
    .createSubMenu('AI 응답 교정 (Refiner)', (m) => {
      m.replaceContentPanel(async (panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          const modeWrap = document.createElement('div'); modeWrap.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;';
          ...
          const modeSel = document.createElement('select'); modeSel.style.cssText = 'width:min(160px,100%);max-width:100%;padding:4px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;';
          ...
          const wrap = document.createElement('div'); wrap.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;gap:10px;width:100%;margin-bottom:12px;';
          const left = document.createElement('div'); left.style.cssText = 'display:flex;flex-direction:column;gap:4px;flex:1;';
    ...
    .createSubMenu('실행 로그', (m) => {
      const renderLogs = (panel) => {
        ...
        const logAnchors = {
          ...
          '추출 기록': 'log-extract',
          '교정 기록': 'log-refiner',
          '모순 기록': 'log-contradiction'
        };

        const makeLogBox = (title, color, items, renderer) => {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            if (C.createSectionAnchor && logAnchors[title]) nd.appendChild(C.createSectionAnchor(logAnchors[title]));
            const hRow = document.createElement('div'); hRow.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;padding-bottom:8px;border-bottom:1px solid #333;margin-bottom:8px;cursor:pointer;';
        ...
        if (C.createShortcutRow && C.createSectionAnchor) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            nd.appendChild(C.createShortcutRow([
              { label: '주입', target: 'log-inject' },
              { label: '추출', target: 'log-extract' },
              { label: '교정', target: 'log-refiner' },
              { label: '모순', target: 'log-contradiction' }
            ], { marginBottom: '0' }));
            if (C.createSmallDivider) nd.appendChild(C.createSmallDivider({ margin: '8px 0 0' }));
          }});
        }
        ...
        makeLogBox('교정 기록', '#ea5', rLog, (clear, i, nd) => { if(clear){ settings.config.urlRefinerLogs[chatKey]=[]; settings.save(); } else { const r = document.createElement('div'); r.style.cssText = 'margin-bottom:6px;border-bottom:1px dashed #222;padding-bottom:4px;font-size:12px;'; r.innerHTML = `<span style="color:${i.isPass?'#4a9':i.isError?'#d66':'#ea5'};">[${i.time}] ${i.isPass?'완료 통과':i.isError?'실패 에러':'교정 교정됨'}</span>${i.reason?`<br><span style="font-size:11px;color:#da7;">이유: ${i.reason}</span>`:''}`; nd.appendChild(r); } });
    ...
    .createSubMenu('세션 상태 관리', (m) => {
      const renderSessionStatus = async (panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          headerRow.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:12px;border-bottom:1px solid #333;padding-bottom:8px;';
          ...
          for (const st of statusList) {
            ...
            row.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;background:#1a1a1a;border:1px solid #333;border-radius:6px;padding:8px 12px;';
            ...
            let cdStr = st.cooldownRem > 0 ? `<span style="color:#d66;">⏳ 쿨다운 ${st.cooldownRem}턴 남음</span>` : `<span style="color:#4a9;">완료 쿨다운 완료</span>`;
            let decayStr = '';
            if (settings.config.decayEnabled) {
      ...
      m.replaceContentPanel((panel) => {
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          testBtn.onclick = async () => {
            const isVertex = (settings.config.autoExtApiType || 'key') === 'vertex';
            if (isVertex ? !settings.config.autoExtVertexJson : !settings.config.autoExtKey) { alert('API 키/JSON 필요.'); return; }
            testBtn.disabled = true; testResult.textContent = '테스트 중...';
            try {
              const r = await C.callGeminiApi('Say "OK" in one word.', { apiType: settings.config.autoExtApiType, key: settings.config.autoExtKey, vertexJson: settings.config.autoExtVertexJson, vertexLocation: settings.config.autoExtVertexLocation, vertexProjectId: settings.config.autoExtVertexProjectId, model: settings.config.autoExtModel, maxRetries: 0 });
              testResult.textContent = r.text ? '완료 성공: ' + r.text.trim().slice(0, 50) : '실패 실패: ' + r.error; testResult.style.color = r.text ? '#4a9' : '#d66';
            } catch(e) { testResult.textContent = '실패 오류: ' + e.message; testResult.style.color = '#d66'; }
          ...
          // Rerank Prompt
          const rplWrap = document.createElement('div'); rplWrap.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin:10px 0 4px;';
    ...
    .createSubMenu('도움말', (m) => {
      m.replaceContentPanel((panel) => {
        const helpAnchors = {
          '로어 주입': 'help-lore',
          '응답 교정 (Refiner)': 'help-refiner',
          '자동 대화 추출': 'help-extract',
          '의미 검색과 재정렬 (Reranker)': 'help-search',
          '파일과 스냅샷': 'help-files',
          '앵커와 버전 이력': 'help-anchor'
        };

        const addHelp = (title, sections) => {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            if (C.createSectionAnchor && helpAnchors[title]) nd.appendChild(C.createSectionAnchor(helpAnchors[title]));
        ...
        if (C.createShortcutRow && C.createSectionAnchor) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            C.setFullWidth(nd);
            nd.appendChild(C.createShortcutRow([
              { label: '빠른 시작', target: 'help-start' },
              { label: '주입', target: 'help-lore' },
              { label: '교정', target: 'help-refiner' },
              { label: '추출', target: 'help-extract' },
              { label: '검색', target: 'help-search' },
              { label: '파일', target: 'help-files' },
              { label: '앵커', target: 'help-anchor' }
            ], { marginBottom: '0' }));
            if (C.createSmallDivider) nd.appendChild(C.createSmallDivider({ margin: '8px 0 0' }));
          }});
        }
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          if (C.createSectionAnchor) nd.appendChild(C.createSectionAnchor('help-start'));
        ...
        addHelp('자동 대화 추출', [
          ...
          { label: '예시', text: '대화 중 "에리는 20살이야" 같은 문장이 나오면 다음 추출 주기에 "에리" 항목에 나이가 기록됨. 관계 변화나 약속도 함께 정리됨.' },
        ...
        addHelp('앵커와 버전 이력', [
          ...
          { label: '예시', text: '"에리=17세"로 앵커 지정해두면 이후 AI가 "18세" 같은 잘못된 서술을 해도 핵심 필드는 유지됨. 자동 추출이 실수로 관계 설정을 지웠다면 이력에서 이전 버전을 복원함.' },
