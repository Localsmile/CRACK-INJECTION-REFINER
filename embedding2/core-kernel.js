...
(function () {
  ...
  const DEFAULTS = {
    ...
    importMaxEntries: 50,
    rerankPrompt: `Given the current chat conversation context, score each lore entry for INJECTION PRIORITY (1-5).
...
{candidates}`
  ...
  const _tokenCaches = {};
  async function getVertexAccessToken(sa, cacheKey) {
  ...
  async function callGeminiApi(prompt, opts) {
    opts = opts || {};
    const apiType = opts.apiType || 'key';
    const key = opts.key || '';
    const vertexJson = opts.vertexJson || '';
    const vertexLocation = opts.vertexLocation || 'global';
    const vertexProjectId = opts.vertexProjectId || '';
    const firebaseScript = opts.firebaseScript || '';
    const firebaseKey = opts.firebaseKey || '';
    const firebaseProjectId = opts.firebaseProjectId || '';
    const firebaseLocation = opts.firebaseLocation || 'global';
    const model = opts.model || 'gemini-3-flash-preview';
    const thinkingConfig = opts.thinkingConfig || {};
    const maxRetries = opts.maxRetries == null ? 1 : opts.maxRetries;
    const responseMimeType = opts.responseMimeType;
    const cacheKey = opts.cacheKey || 'generate';
    ...
    if (isFirebase) {
      // Firebase 모드는 page-context module script를 주입하지 않고 REST로 직접 호출한다.
      const cfg = parseFirebaseConfig(firebaseScript);
      if (!cfg || !cfg.apiKey || !cfg.projectId) {
        return { text: null, status: 0, error: 'Firebase 설정 형식 오류. firebaseConfig 안의 apiKey와 projectId 필요', retries: 0 };
      }
      const fbKey = firebaseKey || key || cfg.apiKey;
      const fbProjectId = firebaseProjectId || cfg.projectId;
      if (!fbKey) return { text: null, status: 0, error: 'Firebase Web API Key 누락', retries: 0 };
      if (!fbProjectId) return { text: null, status: 0, error: 'Firebase projectId 누락', retries: 0 };

      const is3x = model.includes('gemini-3') || model.includes('gemini-2.0-flash-thinking');
      const loc = is3x ? 'global' : (firebaseLocation || 'us-central1');
      url = 'https://firebasevertexai.googleapis.com/v1beta/projects/' + fbProjectId + '/locations/' + loc + '/publishers/google/models/' + model + ':generateContent';
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const r = (isVertex || isFirebase)
          ? await gmFetch(url, { method: 'POST', headers: headers, body: body })
          : await fetch(url, { method: 'POST', headers: headers, body: body }).then(resp => ({ ok: resp.ok, status: resp.status, text: () => resp.text(), json: () => resp.json() }));
        lastStatus = r.status;
        ...
        if (!r.ok) {
          ...
          const text = textPart && textPart.text ? textPart.text : null;
          if (text) return { text: text, status: r.status, error: null, retries: attempt };
          lastError = '응답 파싱 실패';
        }
      } catch (e) {
        lastError = e.message;
  ...
  async function embedTexts(texts, opts) {
    ...
    const vertexLocation = opts.vertexLocation || 'global';
    const vertexProjectId = opts.vertexProjectId || '';
    const firebaseEmbedKey = opts.firebaseEmbedKey || '';
    const model = opts.model || DEFAULTS.embeddingModel;
    const dimensions = opts.dimensions || DEFAULTS.embeddingDimensions;
    const taskType = opts.taskType || DEFAULTS.embeddingTaskType;
    const cacheKey = opts.cacheKey || 'embed';
    ...
    if (isFirebase) {
      ...
      return embedTexts(arr, Object.assign({}, opts, { apiType: 'key', key: firebaseEmbedKey, model: 'gemini-embedding-001' }));
    } else if (isVertex) {
      const sa = parseServiceAccountJson(vertexJson);
      ...
      const r = await gmFetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ instances: arr.map(t => ({ content: t })), parameters: { outputDimensionality: dimensions } })
      });
      ...
      if (arr.length === 1) {
        ...
        const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requests: requests }) });
