// == 인젝터 모듈 4/6 — 추출 파이프라인 ==
// mergeExtractedData, runAutoExtract, _doExtract, extraction status
(async function(){
  'use strict';
  ...
  function prepareExtractedEntry(e) {
    e.gs = (e.imp || 5) + (e.sur || 5) + (e.emo || 5);
    if (settings.config.importanceGating !== false) {
      if (e.gs < (settings.config.importanceThreshold || 12)) return false;
      e.gateScore = e.gs;
    }
    return true;
  }

  async function findExistingEntry(packName, name) {
    return await db.entries.where('packName').equals(packName).and(x => x.name === name).first();
  }

  function snapshotAnchorFields(existing) {
    if (!existing || existing.anchor !== true) return null;
    return {
  ...
  function restoreAnchorFields(existing, snap) {
    if (!snap) return;
    for (const k of Object.keys(snap)) {
      if (snap[k] === undefined) delete existing[k];
      else existing[k] = snap[k];
    }
  }

  function logStatusContradiction(existing, e, chatKey) {
  ...
  function mergeTriggers(existing, e) {
    existing.triggers = [...new Set([...(existing.triggers || []), ...(e.triggers || [])])];
  }

  function mergeCallDelta(existing, e, chatKey) {
    if (!Array.isArray(e.callDelta) || e.callDelta.length <= 0) return;
  ...
  async function mergeExtractedData(entries, url) {
    ...
    for (const e of entries) {
      if (!e.name) continue;
      if (!prepareExtractedEntry(e)) continue;
      processedCount++;
      let existing = await findExistingEntry(packName, e.name);
      if (existing) {
        ...
        const _anchorSnap = snapshotAnchorFields(existing);
        if (!_anchorGuard && ['relationship', 'promise', 'rel', 'prom'].includes(e.type)) {
          logStatusContradiction(existing, e, chatKey);
        }
        mergeTriggers(existing, e);
        ...
        mergeCallDelta(existing, e, chatKey);
        ...
        restoreAnchorFields(existing, _anchorSnap);
        existing.searchStale = true;
        existing.searchStaleAt = Date.now();
        await db.entries.put(existing);
        if (C.invalidateEntryEmbeddings) await C.invalidateEntryEmbeddings(existing.id);
      } else {
  ...
  function extBadgeShow(msg) {
    try {
      if (C.status && C.status.set) C.status.set('extract', msg, { busy: true });
      else C.showStatusBadge(msg);
    } catch(e){}
  }

  async function notifyManual(title, message, tone) {
    const UI = _w.__LoreInjUI;
    if (UI && typeof UI.createAlertDialog === 'function') {
      await UI.createAlertDialog({ title, message, tone: tone || 'info' });
      return;
    }
    alert(message);
  }
  function extBadgeHide() {
    try {
      if (C.status && C.status.clear) C.status.clear('extract');
      else C.hideStatusBadge();
    } catch(e){}
  }
  ...
  async function _doExtract(isManual) {
    ...
    if (missing) { if (isManual) await notifyManual('API 설정 필요', 'API 설정을 먼저 완료하세요.', 'warn'); return; }
    ...
    if (!recentMsgs.length) { if (isManual) await notifyManual('대화 기록 없음', '최근 대화 기록을 찾지 못했어요.', 'warn'); return; }
    ...
    if (settings.config.autoExtIncludeDb) {
      ...
      if (existingEntries.length > 0) {
        ...
        if (existingEntries.length > 30) {
          ...
          const scoreEntry = (e) => {
            let s = 0;
            const hay = C.entrySearchText ? C.entrySearchText(e) + ' ' + (e.triggers || []).join(' ') : (e.name || '') + ' ' + (e.triggers || []).join(' ') + ' ' + (e.embed_text || '');
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        ...
        if (isManual) await notifyManual('추출 완료', cnt + '개 로어를 추출하고 병합했어요.' + embedMsg, 'success');
      } else {
        addExtLog(chatKey, { time: new Date().toLocaleTimeString(), count: 0, msgs: recentMsgs.length, isManual, status: '추출 내용 없음', api: apiLog });
        if (isManual) await notifyManual('추출 내용 없음', '새로 저장할 설정 정보를 찾지 못했어요.', 'info');
      }
    } catch (err) {
      ...
      if (isManual) await notifyManual('추출 실패', err.message, 'error');
    }
  }
