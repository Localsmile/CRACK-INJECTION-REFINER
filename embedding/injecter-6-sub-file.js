      ...
      const renderPackUI = async (panel) => {
        ...
        panel.addBoxedField('', '', { onInit: async (nd) => {
          ...
          for (const pack of packs) {
            ...
            const embBtn = document.createElement('button'); embBtn.textContent = '임베딩'; embBtn.style.cssText = B + 'color:#4a9;border-color:#264;'; embBtn.onclick = async () => { const apiOpts = _w.__LoreInj.buildEmbeddingApiOpts ? _w.__LoreInj.buildEmbeddingApiOpts({ feature: 'embed', chatKey: 'global' }) : { apiType: settings.config.autoExtApiType || 'key', key: settings.config.autoExtKey, vertexJson: settings.config.autoExtVertexJson, vertexLocation: settings.config.autoExtVertexLocation || 'global', vertexProjectId: settings.config.autoExtVertexProjectId, firebaseEmbedKey: settings.config.autoExtFirebaseEmbedKey, model: settings.config.embeddingModel || 'gemini-embedding-001' }; const miss = _w.__LoreInj.isEmbeddingApiConfigured ? !_w.__LoreInj.isEmbeddingApiConfigured() : (apiOpts.apiType === 'vertex' ? !apiOpts.vertexJson : apiOpts.apiType === 'firebase' ? !apiOpts.firebaseEmbedKey : !apiOpts.key); if (miss) { alert(apiOpts.apiType === 'firebase' ? '임베딩용 Gemini API 키 필요.' : '임베딩 API 설정 필요.'); return; } if (!confirm('[' + pack.name + '] 임베딩 생성?')) return; embBtn.disabled = true; const orig = embBtn.textContent; try { const cnt = await C.embedPack(pack.name, apiOpts, (done, total) => { embBtn.textContent = done + '/' + total; }); embBtn.textContent = 'OK' + cnt; setTimeout(() => { embBtn.textContent = orig; embBtn.disabled = false; }, 2000); } catch (e) { embBtn.textContent = 'X'; embBtn.disabled = false; alert('실패:' + e.message); } };
