...
(function () {
  ...
  function createHelpIcon(onClick) {
    const icon = document.createElement('span');
    icon.textContent = '?';
    icon.style.cssText = 'display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;background:#333;color:#888;font-size:10px;font-weight:bold;cursor:pointer;flex-shrink:0;user-select:none;';
    icon.onmouseenter = () => { icon.style.background = '#446'; icon.style.color = '#aaf'; };
    icon.onmouseleave = () => { icon.style.background = '#333'; icon.style.color = '#888'; };
    icon.onclick = (e) => { e.stopPropagation(); if (onClick) onClick(e); };
    return icon;
  }

  function showHelpPopup(content, triggerEl) {
    const existing = document.getElementById('lore-help-popup');
    if (existing) existing.remove();
    const popup = document.createElement('div');
    popup.id = 'lore-help-popup';
    popup.style.cssText = 'position:fixed;z-index:999999;max-width:360px;background:#1a1a1a;border:1px solid #446;border-radius:8px;padding:14px 16px;color:#ccc;font-size:12px;line-height:1.7;box-shadow:0 4px 16px rgba(0,0,0,0.6);word-break:keep-all;';
    ...
    closeBtn.style.cssText = 'position:absolute;top:6px;right:10px;font-size:11px;color:#888;cursor:pointer;';
    closeBtn.onclick = () => popup.remove();
    popup.appendChild(closeBtn);
    setTimeout(() => {
      const handler = (e) => { if (!popup.contains(e.target) && e.target !== triggerEl) { popup.remove(); document.removeEventListener('click', handler); } };
