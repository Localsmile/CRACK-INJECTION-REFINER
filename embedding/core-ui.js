...
(function () {
  ...
  function createHelpMark(text) {
    const q = document.createElement('button');
    q.type = 'button';
    q.textContent = '?';
    q.title = text || '';
    q.style.cssText = 'width:18px;height:18px;border-radius:50%;border:1px solid #446;background:#111;color:#9bd;font-size:11px;line-height:16px;text-align:center;cursor:pointer;padding:0;flex-shrink:0;';
    q.onclick = (ev) => {
  ...
  function makeHelpLabel(label, helpText, cssText) {
    const wrap = document.createElement('div');
    wrap.style.cssText = 'display:flex;align-items:center;gap:5px;';
    const l = document.createElement('div');
    l.textContent = label;
    l.style.cssText = cssText || 'font-size:12px;color:#888;margin-bottom:4px;';
    wrap.appendChild(l);
    if (helpText) wrap.appendChild(createHelpMark(helpText));
    return wrap;
  }
  ...
  function createToggleRow(title, desc, isChecked, onChange) {
    ...
    left.appendChild(tWrap); left.appendChild(d);
  ...
  Object.assign(C, {
    ...
    setFullWidth, createHelpMark, makeHelpLabel, createToggleRow, createApiInput,
    __uiLoaded: true
  });
