  ...
  _w.__LoreInj.registerSubMenu('log', function(modal) {
    modal.createSubMenu('실행 로그', (m) => {
      const renderLogs = (panel) => {
        ...
        const renderCostBox = (panel) => {
          panel.addBoxedField('', '', { onInit: (nd) => {
            ...
            const refresh = () => {
              ...
              // v1.4.0-test.39 B15 후속: '전체' 기간 헤더는 FIFO 5000 이전 데이터까지 포함된
              //   누적 합계(C.getCumulativeCost)를 우선 사용. 상세 표는 여전히 events 기반.
              ...
              // test.42: temporalExtract 라벨 추가 — 시간축 추출 비용을 별도 기능으로 표시(B20 후속 UI).
              ...
              const renderTable = (title, bucket, keyLabel, labelMap) => {
                ...
                for (const [k, v] of rows) {
                  ...
                  const labelText = labelMap && labelMap[k] ? labelMap[k] + ' <span style="color:#666;">(' + escHtml(k) + ')</span>' : escHtml(k);
              ...
              if (!events.length) {
                ...
                html += renderTable('채팅별', byChat, '채팅', null);
                html += '<div style="margin-top:8px;font-size:10px;color:#666;">~ usageMetadata 없어 char/4 추정. — 가격 미등록 모델(직접입력 등) 호출수만 표기, USD 합산 제외.</div>';
        ...
        makeLogBox('교정 기록', '#ea5', rLog, (clear, i, nd) => { if(clear){ settings.config.urlRefinerLogs[chatKey]=[]; settings.save(); } else { const r = document.createElement('div'); r.style.cssText = 'margin-bottom:6px;border-bottom:1px dashed #222;padding-bottom:4px;font-size:12px;'; r.innerHTML = `<span style="color:${i.isPass?'#4a9':i.isError?'#d66':'#ea5'};">[${i.time}] ${i.isPass?'✅ 통과':i.isError?'❌ 에러':'✏️ 교정됨'}</span>${i.reason?`<br><span style="font-size:11px;color:#da7;">💡 ${i.reason}</span>`:''}${renderCostLine(i)}`; nd.appendChild(r); } });
