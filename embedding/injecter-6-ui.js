...
(async function(){
  ...
  // 3) Fallback 기어 버튼 — v1.4.0-test.39에서 비활성화, v1.4.0-test.40에서 dead code 제거.
  //    chasm 텍스트 진입점이 정상 작동하므로 fallback 불필요. 복구 필요 시 git history 참조.

  document.getElementById('lore-inj-gear-btn')?.remove();
  __doModalMenuInit();
  /* gear FAB 비활성화 — chasm 텍스트 진입점만 사용.
     v1.4.0-test.39 B10 fix: 빈 콜백만 돌던 MutationObserver/resize 리스너 제거.
     (ensureGearButton 함수는 호출처가 없어 dead code지만 향후 복구 가능성을 위해 정의는 보존.) */
