...
(function () {
  ...
  async function fetchExternalText(url) {
    ...
    try {
      const resp = await gmFetch(normalized, { method: 'GET', headers: directHeaders });
    ...
    try {
      const r = await fetch(normalized, { method: 'GET', credentials: 'omit' });
    ...
    for (const fu of fallbackUrls) {
      try {
        const resp = await gmFetch(fu, { method: 'GET', headers: { 'Accept': 'text/html,text/plain,*/*' } });
        if (resp && resp.ok) return await resp.text();
        if (resp) errors.push('proxy HTTP ' + resp.status);
      } catch (e) { errors.push('proxy ' + (e.message || String(e))); }
    }
