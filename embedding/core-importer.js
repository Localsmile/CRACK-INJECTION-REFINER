...
(function () {
  ...
  function normalizeSummaryValue(summary, name, state) {
    if (summary && typeof summary === 'object' && !Array.isArray(summary)) {
      const full = clampText(summary.full || summary.compact || summary.micro || '', 260);
      const compact = clampText(summary.compact || full, 120);
      const micro = clampText(summary.micro || (state ? `${name}=${state}` : compact || name), 45);
      return { full, compact, micro };
    }
    const full = clampText(summary || '', 260);
    const compact = clampText(full || state || name || '', 120);
    const micro = clampText(state ? `${name}=${state}` : compact || name || '', 45);
    return { full, compact, micro };
  }
  ...
  function mergeLoreSummary(existingSummary, incomingSummary, name, state) {
    ...
    return {
      full: mergeText(ex.full, inc.full, 260),
      compact: inc.compact || ex.compact,
