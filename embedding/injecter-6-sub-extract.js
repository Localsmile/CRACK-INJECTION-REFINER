    ...
    modal.createSubMenu('추출/변환 설정', (m) => {
      m.replaceContentPanel((panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          nd.appendChild(C.createToggleRow('기존 로어 전송', '전체 JSON 대신 digest/patch 기반으로 보내 중복 방지.', settings.config.autoExtIncludeDb, (v) => { settings.config.autoExtIncludeDb = v; settings.save(); }));
          nd.appendChild(C.createToggleRow('저출력 Patch 모드', '기존 로어는 전체 재출력 대신 변경분만 출력.', settings.config.autoExtPatchMode !== false, (v) => { settings.config.autoExtPatchMode = v; settings.save(); }));
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          urlBtn.onclick = async () => {
            ...
            setBusy(phaseMsg + ' (0초)');
            try {
              const cnt = await C.importFromUrl(urlInp.value.trim(), nameInp.value.trim(), buildGenerationApiOpts({}, { feature: 'urlImport', packName: nameInp.value.trim(), chatKey: (C.getCurrentChatId && C.getCurrentChatId()) || 'global' }), {
                onProgress: (ev) => {
                  if (!ev) return;
                  switch (ev.phase) {
                    case 'fetch:start':
          ...
          tBtn.onclick = async () => {
            ...
            setBusy(phaseMsg + ' (0초)');
            try {
              const cnt = await C.importFromText(ta.value.trim(), nameInp2.value.trim(), buildGenerationApiOpts({}, { feature: 'textImport', packName: nameInp2.value.trim(), chatKey: (C.getCurrentChatId && C.getCurrentChatId()) || 'global' }), {
                onProgress: (ev) => {
