    ...
    modal.createSubMenu('추출/변환 설정', (m) => {
      m.replaceContentPanel((panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          const jd = document.createElement('div'); jd.textContent = '최근 대화 기준으로 참고할 과거 장면을 가벼운 모델이 고름.'; jd.style.cssText = 'font-size:11px;color:#888;margin-bottom:10px;line-height:1.4;'; nd.appendChild(jd);
          ...
          nd.appendChild(jrs);
          const proWarn = document.createElement('div'); proWarn.style.cssText = 'font-size:10px;color:#d96;margin-top:4px;display:none;'; proWarn.textContent = '주의: Gemini 3.1 Pro는 minimal 미지원. low 이상 권장.'; nd.appendChild(proWarn);
