    ...
    modal.createSubMenu('추출/변환 설정', (m) => {
      m.replaceContentPanel((panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          C.setFullWidth(nd);
          nd.appendChild(C.createToggleRow('자동 대화 정리', '대화를 주기적으로 로어에 추가합니다.', settings.config.autoExtEnabled, (v) => { settings.config.autoExtEnabled = v; settings.save(); }));
          nd.appendChild(C.createToggleRow('기존 로어 참고', '이미 저장된 로어를 함께 보내 중복 저장을 줄입니다.', settings.config.autoExtIncludeDb, (v) => { settings.config.autoExtIncludeDb = v; settings.save(); }));
          nd.appendChild(C.createToggleRow('변경분만 저장', '기존 로어를 다시 쓰지 않고 바뀐 내용만 받아 출력 토큰을 줄입니다.', settings.config.autoExtPatchMode !== false, (v) => { settings.config.autoExtPatchMode = v; settings.save(); }));
          ...
          nd.appendChild(C.createToggleRow('중요 장면 기억하기', '나중에 다시 떠올릴 수 있도록 중요한 사건과 약속을 별도로 정리합니다.', settings.config.temporalExtractEnabled !== false, (v) => { settings.config.temporalExtractEnabled = v; settings.save(); }));
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          const jd = document.createElement('div'); jd.textContent = '최근 대화에 맞춰 어떤 과거 장면을 참고할지 가벼운 모델로 고릅니다.'; jd.style.cssText = 'font-size:11px;color:#888;margin-bottom:10px;line-height:1.4;'; nd.appendChild(jd);
          nd.appendChild(C.createToggleRow('AI로 참고 장면 고르기', '규칙만 쓰지 않고 AI가 필요한 과거 장면을 한 번 더 골라줍니다.', settings.config.temporalRecallJudgeEnabled, (v) => { settings.config.temporalRecallJudgeEnabled = v; settings.save(); }));
          ...
          const jml = document.createElement('div'); jml.textContent = '판단용 모델'; jml.style.cssText = 'font-size:11px;color:#999;margin:10px 0 4px;'; nd.appendChild(jml);
          ...
          const jrl = document.createElement('div'); jrl.textContent = '생각 깊이'; jrl.style.cssText = 'font-size:11px;color:#999;margin:10px 0 4px;'; nd.appendChild(jrl);
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          const lblSchema = document.createElement('div'); lblSchema.textContent = '출력 형식(JSON)'; lblSchema.style.cssText = 'font-size:12px;color:#ccc;margin-bottom:4px;'; nd.appendChild(lblSchema);
          const taSchema = document.createElement('textarea'); taSchema.style.cssText = S + 'height:150px;'; nd.appendChild(taSchema);
          const lbl1 = document.createElement('div'); lbl1.textContent = '새 로어 추출 지시문'; lbl1.style.cssText = 'font-size:12px;color:#ccc;margin-bottom:4px;'; nd.appendChild(lbl1);
          const ta1 = document.createElement('textarea'); ta1.style.cssText = S + 'height:150px;'; nd.appendChild(ta1);
          const lbl2 = document.createElement('div'); lbl2.textContent = '기존 로어 참고 지시문'; lbl2.style.cssText = 'font-size:12px;color:#ccc;margin-bottom:4px;'; nd.appendChild(lbl2);
