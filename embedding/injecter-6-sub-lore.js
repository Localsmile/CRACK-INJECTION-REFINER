        ...
        for (const [pk, items] of Object.entries(byPack)) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            ...
            for (const e of items) {
              ...
              saveBtn.onclick = async () => { try { const parsed = JSON.parse(ta.value); const updated = {...e, ...parsed}; await db.entries.put(updated); try { if (C.invalidateEntryEmbeddings) await C.invalidateEntryEmbeddings(updated.id); } catch(_){} alert('수정됨. 검색 준비는 갱신 필요로 표시되며 필요 시 다시 실행하세요.'); Object.assign(e, updated); nameSpan.textContent = '[' + updated.type + '] ' + updated.name; nameSpan.appendChild(embStatusSpan); updateEmbStatus(); toggleEdit(); } catch (err) { alert('JSON 오류: ' + err.message); } };
              delBtn.onclick = async () => { if (confirm('[' + e.name + '] 삭제? 이 로어의 검색 준비와 되돌리기 기록도 함께 삭제됩니다.')) { const report = typeof _w.__LoreInj.deleteEntryData === 'function' ? await _w.__LoreInj.deleteEntryData(e.id) : null; if (!report) { await db.entries.delete(e.id); try { await db.embeddings.where('entryId').equals(e.id).delete(); } catch(_){} try { if (db.entryVersions) await db.entryVersions.where('entryId').equals(e.id).delete(); } catch(_){} const count = await db.entries.where('packName').equals(e.packName).count(); if (count <= 0) await db.packs.delete(e.packName); else await db.packs.update(e.packName, { entryCount: count }); } const remaining = report ? report.remaining : await db.entries.where('packName').equals(e.packName).count(); if (remaining <= 0) { row.remove(); m.replaceContentPanel(renderLoreUI, '로어 관리'); } else { row.remove(); title.textContent = pk + ' (' + remaining + '개)'; } } };
