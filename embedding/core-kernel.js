...
{candidates}`
  ...
  // test.42 보강(B19 후속): AbortSignal forwarding 지원.
  //   GM_xmlhttpRequest 반환 객체에 abort() 메서드 있음 — signal.abort 시 호출해
  //   실제 네트워크 요청 자체를 취소(judge timeout 후 background 누수 제거).
  //   fetch fallback도 동일하게 signal 전달.
  ...
  async function callGeminiApi(prompt, opts = {}) {
    ...
    // 비용 추적 헬퍼. costContext가 있고 core-pricing이 로드되었을 때만 기록. 실패해도 원본 호출에 영향 없음.
    const _trackCost = (usageMeta, promptText, outText) => {
      ...
      try {
        ...
        if (usageMeta && (usageMeta.promptTokenCount != null || usageMeta.candidatesTokenCount != null)) {
          ...
          // usageMetadata가 없으면 char/4 대략 추정 (Gemini 토큰화 근사치)
          inTok = Math.ceil(String(promptText || '').length / 4);
          outTok = Math.ceil(String(outText || '').length / 4);
          estimated = true;
        }
        return core.recordApiCost({
          chatKey: costContext.chatKey || 'global',
          feature: costContext.feature || 'unknown',
    ...
    if (isFirebase) {
      ...
      // test.42 보강(B19 후속): Firebase SDK는 generateContent에 signal 미지원이라
      //   호출 시점에 aborted 여부만 확인. 호출 중 abort는 차단 불가(SDK 한계).
  ...
  async function embedTexts(texts, opts = {}) {
    ...
    // 임베딩 비용 추적 헬퍼. usageMetadata가 비어 있으므로 char/4 추정 + estimated:true.
    // costContext 미지정 시 { feature: 'embed', chatKey: 'global' }로 폴백 → 수동 UI 임베딩도 누락 없이 집계.
