...
(async function(){
  ...
  function mergeInject(existing, incoming, name, state) {
    if (!incoming || typeof incoming !== 'object') return existing;
    const base = existing && typeof existing === 'object' ? JSON.parse(JSON.stringify(existing)) : {};
    const mergedSummary = C.mergeLoreSummary ? C.mergeLoreSummary(base, incoming, name, state) : null;
    return {
      full: mergedSummary ? mergedSummary.full : (incoming.full || base.full || ''),
      compact: mergedSummary ? mergedSummary.compact : (incoming.compact || base.compact || ''),
