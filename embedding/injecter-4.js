  ...
  async function _doExtract(isManual) {
    ...
    if (settings.config.autoExtIncludeDb) {
      ...
      if (existingEntries.length > 0) {
        if (settings.config.autoExtPatchMode !== false) {
          entriesText = buildExistingLoreDigest(existingEntries, context, settings.config.autoExtDbDigestLimit || 40);
        } else {
    ...
    const extractSchema = (settings.config.autoExtIncludeDb && settings.config.autoExtPatchMode !== false) ? (DEFAULT_AUTO_EXTRACT_PATCH_SCHEMA || tpl.schema) : tpl.schema;
    const prompt = personaPrefix + promptTpl.replace('{context}', context).replace('{entries}', entriesText).replace('{schema}', extractSchema);
    ...
    for (let bi = 0; bi < batches.length; bi++) {
      ...
      if (settings.config.autoExtIncludeDb) {
        ...
        if (existing.length > 0) {
          if (settings.config.autoExtPatchMode !== false) {
            entriesText = buildExistingLoreDigest(existing, context, settings.config.autoExtDbDigestLimit || 40);
          } else {
