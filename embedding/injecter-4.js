...
(async function(){
  ...
  function buildExistingLoreDigest(entries, context, limit) {
    const selected = selectExistingForContext(entries, context, limit || 40);
  ...
  async function runTemporalExtractPass(opts = {}) {
    ...
    try {
      ...
      const res = await C.callGeminiApi(prompt, { ...apiOpts, responseMimeType: 'application/json', costContext: { feature: 'temporalExtract', chatKey: chatKey || 'global', packName: await getAutoExtPackForUrl(url) } });
  ...
  async function _doExtract(isManual) {
    ...
    if (settings.config.autoExtIncludeDb) {
      ...
      if (existingEntries.length > 0) {
        // v1.4.0-test.47: digest 분기 제거. patch 모드 ON/OFF와 무관하게 입력은 동일하게 풀바디로 전송.
        // 차이는 오직 출력 스키마 (PATCH_SCHEMA vs full SCHEMA)에서만 발생. 토큰 폭주 방지 상한은 20개.
        let filtered = existingEntries;
        if (existingEntries.length > 20) {
          filtered = selectExistingForContext(existingEntries, context, 20);
        }
    ...
    try {
      const apiOpts = {
        ...
        costContext: { feature: 'autoExtract', chatKey: chatKey || 'global', packName: await getAutoExtPackForUrl(_url) }
      };
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        ...
        addExtLog(chatKey, { time: new Date().toLocaleTimeString(), count: cnt, msgs: recentMsgs.length, isManual, status: '성공', api: apiLog, model: _extModel, elapsedMs: _extElapsedMs, cost: _extCost });
    ...
    for (let bi = 0; bi < batches.length; bi++) {
      ...
      if (settings.config.autoExtIncludeDb) {
        ...
        if (existing.length > 0) {
          // v1.4.0-test.47: 배치 추출도 동일. 입력 분기 제거, 풀바디 통일, 상한 20.
          const filtered = existing.length > 20 ? selectExistingForContext(existing, context, 20) : existing;
          const clean = filtered.map(({ id, packName, project, enabled, ...rest }) => rest);
          entriesText = JSON.stringify(clean, null, 2);
        }
      }
      ...
      for (let attempt = 0; attempt < maxAttempts && !ok; attempt++) {
        ...
        try {
          const apiOpts = {
            ...
            costContext: { feature: 'batchExtract', chatKey: chatKey || 'global', packName: await getAutoExtPackForUrl(_url) }
          };
          const _bt0 = Date.now();
          const res = await C.callGeminiApi(prompt, apiOpts);
