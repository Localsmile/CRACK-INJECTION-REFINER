...
(function () {
  ...
  function recordApiCost(ev) {
    ...
    const rec = {
      ...
      packName: ev.packName ? String(ev.packName) : '',
    ...
    if (rec.packName) {
      cumul.byPack = cumul.byPack || {};
      const bp = cumul.byPack[rec.packName] = cumul.byPack[rec.packName] || { usd: 0, count: 0 };
      bp.usd += Number(rec.usd) || 0; bp.count++;
    }
    _persistCumul();
