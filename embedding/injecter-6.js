      ...
      const renderMerge = async (panel) => {
        ...
        for (const [gi, grp] of state.groups.entries()) {
          panel.addBoxedField('', '', { onInit: (nd) => {
            ...
            const typeWarn = types.length > 1 ? '⚠️ 타입 불일치: ' + types.join(', ') : '타입: ' + types[0];
            ...
            for (const e of grp.entries) {
              ...
              row.innerHTML = '<span style="color:#ccc;font-weight:bold;">[' + e.type + '] ' + e.name + '</span> <span style="color:#888;">(' + e.packName + ')</span>' + (e.anchor ? ' <span style="color:#fc4;">⚓</span>' : '') + '<br><span style="font-size:10px;">' + safeSum + '</span>';
              nd.appendChild(row);
            }
            ...
            const buildPreview = async () => {
              ...
              try {
                ...
                preview.innerHTML = '<div style="color:' + color + ';font-weight:bold;margin-bottom:6px;">병합 미리보기 (summary ' + sumLen + '자' + (over ? ' — ⚠️ 상한(' + state.maxChars + ') 초과' : '') + ')</div><pre style="white-space:pre-wrap;margin:0;font-size:11px;color:#ccc;">' + JSON.stringify(mergedDraft, null, 2).replace(/</g, '&lt;') + '</pre>';
              } catch(e) { preview.textContent = '❌ ' + e.message; mergedDraft = null; }
            };
            ...
            execBtn.onclick = async () => {
              ...
              if (sumLen > state.maxChars) {
                if (!confirm('summary가 상한(' + state.maxChars + '자)을 ' + (sumLen - state.maxChars) + '자 초과. 그래도 적용?')) return;
              }
              if (types.length > 1) {
                if (!confirm('타입 불일치 (' + types.join(', ') + '). 주제 경계가 무너질 수 있음. 계속?')) return;
              }
              if (!confirm(grp.entries.length + '개 엔트리를 하나로 병합할 것? (Undo 지원)')) return;
              try {
