    ...
    modal.createSubMenu('API 설정', (m) => {
      m.replaceContentPanel((panel) => {
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          apiSummary.textContent = '현재 방식: ' + apiTypeLabel + ' · 추출/정리, 변환, 시간축 추출이 이 연결 사용함.';
          ...
          const modelNote = document.createElement('div'); modelNote.textContent = '자동/수동 추출, 지식 변환, 중요 장면 추출은 추출/정리용 모델 사용함. 후보 재정렬과 응답 교정은 별도 모델 사용 가능.'; modelNote.style.cssText = 'font-size:11px;color:#888;margin-bottom:8px;line-height:1.4;'; nd.appendChild(modelNote);
          const ml = document.createElement('div'); ml.textContent = '추출/정리용 모델'; ml.style.cssText = 'font-size:11px;color:#999;margin:10px 0 4px;'; nd.appendChild(ml);
          ...
          const rml = document.createElement('div'); rml.textContent = '후보 재정렬 모델'; rml.style.cssText = 'font-size:11px;color:#999;margin:10px 0 4px;'; nd.appendChild(rml);
          ...
          const promptHead = document.createElement('div'); promptHead.textContent = '고급 지시문'; promptHead.style.cssText = 'font-size:13px;color:#ccc;font-weight:bold;margin:14px 0 8px;padding-top:10px;border-top:1px solid #333;'; nd.appendChild(promptHead);
          ...
          const rpl = document.createElement('div'); rpl.textContent = '후보 재정렬 지시문'; rpl.style.cssText = 'font-size:11px;color:#999;';
          ...
          rplBtn.onclick = () => { if(confirm('후보 재정렬 지시문 기본값 복구?')) { settings.config.rerankPrompt = C.DEFAULTS.rerankPrompt; settings.save(); rpta.value = C.DEFAULTS.rerankPrompt; } };
  
          const rfl = document.createElement('div'); rfl.textContent = '응답 교정 모델'; rfl.style.cssText = 'font-size:11px;color:#999;margin:14px 0 4px;'; nd.appendChild(rfl);
