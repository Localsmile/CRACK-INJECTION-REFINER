          ...
          panel.addBoxedField('', '', { onInit: (nd) => {
            ...
            const mkLlmMerge = async (entries) => {
              ...
              const prompt = '다음은 중복으로 판단된 로어 엔트리들이다. 하나의 로어 JSON으로 병합하라.\n' +
                '원칙:\n1. 핵심 정보는 누락하지 않는다\n2. 불필요한 반복·수식어는 제거한다\n3. summary는 {full, compact, micro} 객체로 병합한다. full은 ' + state.maxChars + '자 이내 self-contained, compact는 관계/상태/훅 보존, micro는 안정 회상 핸들+현재 상태\n4. embed_text에는 이름/별칭/관계어/사건 원인/이해관계/장소/미해결 훅을 포함한다\n5. triggers는 필수 키워드만 유지 (최대 8개)\n6. detail/callState/timeline/entities는 서로 다른 하위 정보를 통합\n7. type은 가장 구체적인 것 유지\n8. 출력은 순수 JSON 객체 하나만.\n\n' +
