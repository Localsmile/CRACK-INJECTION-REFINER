          ...
          panel.addBoxedField('', '', { onInit: (nd) => {
            ...
            const mkLlmMerge = async (entries) => {
              ...
              const apiOpts = _w.__LoreInj.buildGenerationApiOpts
                ? _w.__LoreInj.buildGenerationApiOpts('autoExtModel', 'autoExtCustomModel', { maxRetries: 2 }, { feature: 'merge', chatKey: 'global' })
                : {
              ...
              try {
                ...
                try {
                  const apiOpts = _w.__LoreInj.buildEmbeddingApiOpts
                    ? _w.__LoreInj.buildEmbeddingApiOpts({ feature: 'embed', chatKey: 'global' })
                    : { apiType: settings.config.autoExtApiType || 'key', key: settings.config.autoExtKey, vertexJson: settings.config.autoExtVertexJson, vertexLocation: settings.config.autoExtVertexLocation || 'global', vertexProjectId: settings.config.autoExtVertexProjectId, firebaseEmbedKey: settings.config.autoExtFirebaseEmbedKey, model: settings.config.embeddingModel || 'gemini-embedding-001' };
                  const hasApi = _w.__LoreInj.isEmbeddingApiConfigured ? _w.__LoreInj.isEmbeddingApiConfigured() : (apiOpts.apiType === 'vertex' ? !!apiOpts.vertexJson : apiOpts.apiType === 'firebase' ? !!apiOpts.firebaseEmbedKey : !!apiOpts.key);
