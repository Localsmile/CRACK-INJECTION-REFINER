...
(function(){
  ...
  const VER = '1.4.0-test.44';
  const OOC_FORMAT_VERSION = 'v1.4.0-ooc-context-lore';
  const CHANGELOG = [
    '1.4.0-test.44 Low 라운드 (B29): core-memory.extractSceneKeywords 한·영 병행. 영문 위치 패턴 (at/in/on + room/cafe/park 등 35개 접미사), 영문 액션 21개, 영문 감정 23개 추가. 영문 단어는 단어 경계 매칭으로 false positive 차단.',
    '1.4.0-test.43 Medium 라운드 (B25/B26/B28): core-memory.detectActiveCharacters 영문 캐릭 이름 minLen 3→2 (2자 영문 이름 활성 감지 포함, 단어 경계 정규식이 false positive 차단), core-memory.temporalRecallCueScore 영문 cue 단어 경계 ("before"→"beforehand" 등 false positive 차단), core-format.cfCompact 호칭 정보 보존 강화 (nicknames from 보존 + callState 폴백)',
    '1.4.0-test.42 High fix: 임베딩 stale 검증을 모든 임베딩 모델로 일반화(B22), 로어 예산 계획에 같은 주체 번들링 도입(B23), kernel callGeminiApi/gmFetch에 AbortSignal forwarding 추가(B19 보강 — 실제 네트워크 요청 취소), 시간축 추출 비용 라벨 UI 표시(temporalExtract)',
    '1.4.0-test.41 Critical fix: judge timeout AbortController(background fetch 취소 시도), 자동 embeddingWeight 조정 opt-in 게이트(autoTuneEmbeddingWeight 명시적으로 true 시만), 시간축 추출 비용 feature 분리(temporalExtract)',
    '1.4.0-test.40 안정화 2차: 교정 중복 지문을 채팅별로 분리(채팅간 오탄 방지), 로어 병합 탐색의 대규모 그룹화 재귀를 반복문으로 전환, 비활성 fallback 기어 버튼 완전 제거',
    '1.4.0-test.39 안정화: 교정 워치독 누수/setInterval 누수 버그 수정, 임베딩 변종 모델 단가 폴백 추가, API 비용 전체 기간 누적 합계를 FIFO 제한 없이 보존, 메뉴 MutationObserver debounce',
...
{context}`;
  ...
  const DEFAULT_AUTO_EXTRACT_PATCH_SCHEMA = `[<br>  {<br>    "op": "add",<br>    "entry": {<br>      "type": "character|location|item|event|concept|setting|rel|prom",<br>      "name": "Entity Name",<br>      "triggers": ["keyword1", "CharA&&CharB"],<br>      "summary": { "full": "self-contained continuity", "compact": "state + hook", "micro": "name=status" },<br>      "inject": { "full": "max 120 chars", "compact": "max 70 chars", "micro": "max 35 chars" },<br>      "embed_text": "names aliases relationship terms causes stakes locations hooks",<br>      "state": "current situation",<br>      "timeline": { "eventTurn": 0, "relativeOrder": "current|past|foreshadow", "sceneLabel": "", "observedRecency": "recent|old|unknown" },<br>      "entities": [],<br>      "eventHistory": [{"turn": 0, "summary": "new concrete event", "imp": 8, "emo": 8}],<br>      "imp": 5, "sur": 5, "emo": 5<br>    }<br>  },<br>  {<br>    "op": "patch",<br>    "id": 0,<br>    "reason": "short reason",<br>    "set": {<br>      "state": "",<br>      "summary": { "full": "", "compact": "", "micro": "" },<br>      "inject": { "full": "", "compact": "", "micro": "" },<br>      "callState": {},<br>      "timeline": {},<br>      "entities": [],<br>      "cond": ""<br>    },<br>    "append": {<br>      "triggers": [],<br>      "eventHistory": [{"turn": 0, "summary": "new concrete event", "imp": 8, "emo": 8}],<br>      "callHistory": [{"turn": 0, "from": "A", "to": "B", "term": "current", "prevTerm": "previous"}]<br>    }<br>  }<br>]`;
