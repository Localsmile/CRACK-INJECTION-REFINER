...
12. CONTEXT-SAFE MERGE PATCHES (non-anchored entries):
    - The Existing Lore Database may be partial when the DB is large. Never assume omitted old facts are false.
    - For each existing entry, output ONLY changed/new slots; the merge layer preserves omitted slots.
    - If updating summary/inject, preserve the existing kernel and APPEND the new scene detail. Do NOT replace a long continuity record with only the latest scene.
