...
Conversation Log:
{context}`;<br><br>  // v1.4.0-test.47: 앨 promptWithDb 본문을 LEGACY로 보존. settings.load() 시 정확히 일치하는 앨 본문이면 새 DEFAULT로 자동 업그레이드. 토씨 하나라도 사용자가 손볼면 norm 불일치 → 보존.<br>  const LEGACY_AUTO_EXTRACT_PROMPTS_WITH_DB = [<br>    `You are a Lore Archivist for RP.
Analyze the following conversation log ALONGSIDE the EXISTING Lore Database.
Extract NEW or UPDATED lore, character developments, relationship changes, and promise status updates.
