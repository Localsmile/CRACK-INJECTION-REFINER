...
(function () {
  ...
  async function processQueue() {
    ...
    R.lastState = { state: 'running', detail: 'AI 교정 중', at: Date.now(), queue: refineQueue.length, busy: true };

    const item = refineQueue.shift();
    const fingerprints = R.getProcessedFingerprints();
    if (fingerprints.has(item.fingerprint)) {
      R.workerBusy = false;
      R.lastState = { state: 'skipped', detail: '이미 교정한 응답', at: Date.now(), queue: refineQueue.length, busy: false };
    ...
    R.workerBusy = false;
    if (!R.lastState || R.lastState.state === 'running') R.lastState = { state: 'idle', detail: '교정 완료', at: Date.now(), queue: refineQueue.length, busy: false };
