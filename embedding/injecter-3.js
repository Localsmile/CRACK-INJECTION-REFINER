...
(async function(){
  ...
  const defaultSettings = {
    ...
    deepSeekJsonSystemPrompt: 'You are a strict JSON API for an RP lore memory tool. Return only one valid json object. Do not output markdown fences, explanations, comments, or trailing text. Preserve the language of the source conversation. Follow the exact object shape requested by the user.',
    deepSeekExtractJsonPrompt: 'DeepSeek JSON output rule: return exactly {"entries":[...]} as one valid json object. Put every add, patch, or full lore entry object inside entries. If nothing changed, return exactly {"entries":[]}.',
    deepSeekTemporalJsonPrompt: 'DeepSeek JSON output rule: return exactly {"entries":[...]} as one valid json object. Put every add, patch, or timeline_event object inside entries. If no important scene changed, return exactly {"entries":[]}.',
    deepSeekImportJsonPrompt: 'DeepSeek JSON output rule: return exactly {"entries":[...]} as one valid json object. Put every converted lore entry inside entries. If no useful lore exists, return exactly {"entries":[]}.',
