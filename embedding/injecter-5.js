...
(async function(){
  ...
  async function inject(userInput) {
    ...
    const topScored = _filteredScored.slice(0, config.maxEntries || 4);
    ...
    const temporalHints = C.formatTemporalHints ? C.formatTemporalHints(topEntries, { currentTurn: turnCounter, sceneId: chatKey, activeNames, budget: config.temporalHintChars || C.DEFAULTS.temporalHintChars || 180 }) : '';
    ...
    try {
      for (const e of (fmtResult.included || [])) {
    ...
    try {
      ...
      st.lastInjected = (fmtResult.included || []).map(e => e.name); _ls.setItem(sk, JSON.stringify(st));
    } catch(e) {}
