...
(function () {
  ...
  const HELP_TEXTS = {
    ...
    '한 번에 넣을 로어': '한 입력에 포함할 로어 최대 개수. 예: 4이면 후보 중 최대 4개만 삽입.',
    '기본 삽입 문자 수': '평상시 목표 삽입량. 예: 300이면 관련 로어를 300자 안팎으로 맞춤.',
    '최대 삽입 문자 수': '강하게 관련된 로어가 많을 때 허용할 최대 삽입량. 예: 700이면 예산 초과 시 700자 근처에서 멈춤.',
    '장면 상태 문자 수': '현재 장소, 등장 인물, 미해결 상태를 담는 별도 장면 상태 블록의 크기. 예: 300이면 장면 상태를 300자 안팎으로 압축.',
    '삽입 흔적 자동 정리': '일정 분량이 지나면 이전에 삽입한 로어 부분만 원문 메시지에서 제거함.',
    '정리 기준 문자 수': '삽입된 메시지 뒤로 이 문자 수만큼 새 대화가 쌓이면, 이전 유저 메시지에서 로어 삽입 부분만 지움. 예: 7000이면 이후 대화 합계가 7000자를 넘을 때 정리.',
  ...
  const OPTION_SEARCH_BY_PAGE = {
    ...
    'knowledge-text': ['텍스트 변환', '원문', '팩 이름'],
    'knowledge-file': ['파일 관리', '로어 가져오기', '직접 JSON 입력', 'JSON 파일 선택', '입력 내용 추가'],
    'knowledge-url': ['URL 변환', 'URL', '팩 이름'],
    ...
    file: ['전체 로어팩', '내보내기', '검색 준비 정리', '임베딩', '삭제'],
  ...
  function decorateHelp(content) {
    ...
    candidates.forEach((el) => {
      ...
      if (API_WARNING_ICON_LABELS.has(key)) return;
