    ...
    modal.createMenu('로어 설정', (m) => {
      m.replaceContentPanel(async (panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          nd.appendChild(C.createToggleRow('의미로 찾기', '단어가 정확히 같지 않아도 관련 로어를 찾습니다. API와 검색 준비가 필요합니다.', settings.config.embeddingEnabled, (v) => { settings.config.embeddingEnabled = v; settings.save(); }));
          ...
          emSel.onchange = () => { settings.config.embeddingModel = emSel.value; settings.save(); alert('모델 변경됨. 기존 로어의 검색 준비를 다시 실행해야 정확합니다.'); };
          emRow.appendChild(emL); emRow.appendChild(emSel); nd.appendChild(emRow);

          nd.appendChild(C.createToggleRow('추출 후 검색 준비', '새로 저장한 로어를 의미 검색에 쓸 수 있게 자동으로 준비합니다.', settings.config.autoEmbedOnExtract !== false, (v) => { settings.config.autoEmbedOnExtract = v; settings.save(); }));
          nd.appendChild(C.createToggleRow('오래된 기억 다시 챙기기', '한동안 언급되지 않은 중요 로어도 필요하면 다시 넣습니다.', settings.config.decayEnabled, (v) => { settings.config.decayEnabled = v; settings.save(); }));
          nd.appendChild(C.createToggleRow('AI로 후보 다시 고르기', '검색된 로어 중 현재 장면에 더 맞는 것을 AI가 다시 정렬합니다.', settings.config.rerankEnabled || false, (v) => { settings.config.rerankEnabled = v; settings.save(); }));
        }});
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          nd.appendChild(C.createToggleRow('호칭 정보', '캐릭터들이 서로를 어떻게 부르는지 함께 전달합니다.', settings.config.honorificMatrixEnabled !== false, (v) => { settings.config.honorificMatrixEnabled = v; settings.save(); }));
          nd.appendChild(C.createToggleRow('첫 만남/재회 관리', '처음 만나는 사이인지, 오랜만에 다시 만난 사이인지 등을 자동으로 알려줍니다.', settings.config.firstEncounterWarning !== false, (v) => { settings.config.firstEncounterWarning = v; settings.save(); }));
        }});
