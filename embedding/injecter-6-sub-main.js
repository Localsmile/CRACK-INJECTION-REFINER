    ...
    modal.createMenu('로어 설정', (m) => {
      m.replaceContentPanel(async (panel) => {
        ...
        panel.addBoxedField('', '', { onInit: (nd) => {
          ...
          nd.appendChild(C.createToggleRow('다음 턴 후보 준비', '현재 대화를 보고 다음 삽입에 쓸 후보 순서를 백그라운드로 준비함.', settings.config.rerankEnabled || false, (v) => { settings.config.rerankEnabled = v; settings.save(); }));
        }});
